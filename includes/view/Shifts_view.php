...
function Shift_editor_info_render($shift) {
  ...
  if ($shift['created_by_user_id'] != null)
    $info[] = sprintf(glyph('plus') . _("created at %s by %s"), date('Y-m-d H:i', $shift['created_at_timestamp']), User_Nick_render(findUserById($shift['created_by_user_id'])));
  if ($shift['edited_by_user_id'] != null)
    $info[] = sprintf(glyph('pencil') . _("edited at %s by %s"), date('Y-m-d H:i', $shift['edited_at_timestamp']), User_Nick_render(findUserById($shift['edited_by_user_id'])));
  return join('<br />', $info);
}
  ...
  foreach ($shift['NeedAngels'] as $needed_angeltype) {
    ...
    foreach ($shift['ShiftEntry'] as $shift_entry) {
      if ($shift_entry['TID'] == $needed_angeltype['TID']) {
        $entry = User_Nick_render(findUserById($shift_entry['UID']));
